<template>
    <div class="w-full min-h-screen bg-gray-100 px-6">
        ...
        <div class="max-w-screen-xl mx-auto pt-6 pb-20">
                    ...
                    <div class="w-full mt-4 grid grid-cols-12">
                        <div class="col-span-4">
                            <div class="w-full h-20 flex items-center">
                                <p class="text-gray-600">ຊື່ນັກສຶກສາ:</p>
                            </div>
                        </div>
                        <div class="col-span-8 flex flex-col gap-2">
                            <div v-if="loading" class="w-full h-20 border border-gray-300 rounded-md flex justify-start items-center gap-4 pl-2">
                                <div class="w-16 h-16 bg-gray-200 rounded-full">
                                    <n-skeleton height="100%" width="100%" circle/>
                                </div>
                                <n-skeleton height="20px" width="100px" :sharp="false" />
                            </div>
                            <div v-for="(item, index) in currentThesis?.thesis_student_member_lists" :key="index" class="w-full h-20 border border-gray-300 rounded-md flex justify-start items-center gap-4 pl-2">
                                <div class="w-12 h-12 md:w-16 md:h-16 shrink-0 bg-gray-200 rounded-full">
                                    <img :src="item?.image_url" alt="student_profile" class="w-full h-full object-cover rounded-full">
                                </div>
                                <p class="text-gray-600">{{ item?.student?.student_firstname }} {{ item?.student?.student_lastname }}</p>
                            </div>
                        </div>
            ...
            <student-thesis-recommend-thesis v-if="currentThesis?.major_id" :majorId="currentThesis?.major_id" :thesisId="currentThesis?.thesis_id" thesisType="student"/>
        </div>
    </div>
</template>
...
const breadcrumbItems = [
    ...
    { text: 'ບົດຈົບຂອງນັກສຶກສາ', path: '/student-thesis' },
    { text: 'ລາຍລະອຽດ' }
]
