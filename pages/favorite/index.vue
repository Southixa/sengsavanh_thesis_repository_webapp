<template>
    <div class="w-full min-h-screen bg-gray-100 px-6">
        ...
        <div v-if="!isAuth()" class="max-w-screen-xl mx-auto flex justify-center items-center pt-6">
            <n-empty description="ບໍ່ມີຂໍ້ມູນ" class="mt-8" size="small">
                <template #extra>
                </template>
            </n-empty>
        </div>
        <div v-else class="max-w-screen-xl mx-auto pt-6">
            <div class="w-full">
...
const filterThesisByCurrentUserLiked = (thesisList, userLikedThesisList) => {

    const likedThesisIds = new Set(userLikedThesisList.map(item => item.thesis_id));
    
    return thesisList.filter(thesis => likedThesisIds.has(thesis.thesis_id));
}

const handleFilterThesis = async () => {
    filterDataLoading.value = true
    const resFilterThesisByCheckbox = await getAllByFilter(majorFilterList.value, degreeFilterList.value, yearFilterList.value, searchKeyword.value, "all")
    const resFilterThesisByDropdown = filterThesisByCurrentDropdown(resFilterThesisByCheckbox)
    const resFilterThesisByCurrentUserLiked = filterThesisByCurrentUserLiked(resFilterThesisByDropdown, userLikedThesisList.value)
    thesisList.value = resFilterThesisByCurrentUserLiked;
    // console.log("result on handleFilterThesis =>");
    // console.log(resFilterThesisByCurrentUserLiked);
